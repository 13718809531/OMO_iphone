...
- (void)doStartByCaller
{
    ...
    option.apnsContent = @"视频聊天请求";//[NSString stringWithFormat:@"%@请求", self.callInfo.callType == NIMNetCallTypeAudio ? @"网络通话" : @"视频聊天"];
    option.apnsSound = @"video_chat_tip_receiver.aac";
    [self fillUserSetting:option];
    
    option.videoCaptureParam.startWithCameraOn = TRUE;//(self.callInfo.callType == NIMNetCallTypeVideo);
    
    __weak typeof(self) wself = self;
    
    [[NIMAVChatSDK sharedSDK].netCallManager start:callees type:2 option:option completion:^(NSError *error, UInt64 callID) {
        if (!error && wself) {
...
- (void)fillVideoCaptureSetting:(NIMNetCallVideoCaptureParam *)param
{
    ...
    param.videoCaptureOrientation = NIMVideoOrientationDefault;
