...
- (void)playWithUrl:(NSURL *)url showView:(UIView *)showView
{
    ...
    _oldFrame = showView.frame;
    _showView = showView;
    
    //这里自己写需要保存数据的路径
    NSString *cachesPath = [NSSearchPathForDirectoriesInDomains(NSCachesDirectory, NSUserDomainMask, YES) lastObject];
    NSString *name = [OMOIphoneManager sharedData].kangFuBao_name;
    NSString *movePath = [cachesPath stringByAppendingPathComponent:[NSString stringWithFormat:@"%@.videos",name]];
    ...
    if (![fileManager fileExistsAtPath:movePath]) {
        ...
        isCreat = [fileManager createDirectoryAtPath:movePath withIntermediateDirectories:YES attributes:nil error:&error];
    }else{
    ...
    if(isCreat){
        ...
        NSString *lastPath = [[url absoluteString] componentsSeparatedByString:@"/"].lastObject;
        NSString *copyPath = [movePath stringByAppendingPathComponent:lastPath];
    ...
    if (IOS_VERSION < 7.0 || ![str hasPrefix:@"http"]) {
        ...
        self.currentPlayerLayer.frame = CGRectMake(0, 0, showView.bounds.size.width, showView.lwb_height-44);
        _isLocalVideo = YES;
        ...
        self.currentPlayerLayer       = [AVPlayerLayer playerLayerWithPlayer:self.player];
        self.currentPlayerLayer.frame = CGRectMake(0, 0, showView.lwb_width, showView.lwb_height-44);
        _isLocalVideo = NO;
...
{
    _navBar.hidden = YES;
    _showView.frame = CGRectMake(0, 0, SCREENW, SCREENH);
    
    self.currentPlayerLayer.transform = CATransform3DMakeRotation(M_PI/2, 0, 0, 1);
    self.currentPlayerLayer.frame = CGRectMake(0, 0, SCREENW, SCREENH);
    
    if([self.delegate respondsToSelector:@selector(omo_screenDirectionWithIsFull:)]){
        
        [self.delegate omo_screenDirectionWithIsFull:YES];
    }
}

- (void)halfScreen
{
    if(_navBar.isHidden == NO)return;
    
    _navBar.hidden = NO;
    _showView.frame = _oldFrame;
    
    self.currentPlayerLayer.transform = CATransform3DIdentity;
    self.currentPlayerLayer.frame = CGRectMake(0, 0, _showView.lwb_width, _showView.lwb_height);
    
    if([self.delegate respondsToSelector:@selector(omo_screenDirectionWithIsFull:)]){
        
        [self.delegate omo_screenDirectionWithIsFull:NO];
    }
}
    ...
    self.playbackTimeObserver = [self.player addPeriodicTimeObserverForInterval:CMTimeMake(1, 1) queue:NULL usingBlock:^(CMTime time) {
        ...
        if (strongSelf.current != current) {
            ...
            if(strongSelf.current == strongSelf.duration){
                ...
                if([strongSelf.delegate respondsToSelector:@selector(omo_resumeDidEnd)]){
                    
                    [strongSelf.delegate omo_resumeDidEnd];
                }
            }
            [[NSNotificationCenter defaultCenter] postNotificationName:kTBPlayerProgressChangedNotification object:nil];
        }
...
- (void)buildVideoNavBar
{
    ...
    _navBar.frame = CGRectMake(0, self.showView.lwb_height - 44, self.showView.lwb_width, 44);
    ...
    if (!self.totolTimeLabel) {
        ...
        _totolTimeLabel.frame = CGRectMake(_navBar.lwb_width-52-15, 0, 52, 44);
    ...
    if (!self.videoProgressView) {
        ...
        _videoProgressView.frame = CGRectMake(62+30, 21, _navBar.lwb_width-124-44, 20);
    ...
    if (!self.playSlider) {
        ...
        _playSlider.frame = CGRectMake(62+30, 0, _navBar.lwb_width-124-44, 44);
    ...
    if (!self.screenBUtton) {
        ...
        _screenBUtton.frame = CGRectMake(_navBar.lwb_width - 44, 0, 44, 44);
