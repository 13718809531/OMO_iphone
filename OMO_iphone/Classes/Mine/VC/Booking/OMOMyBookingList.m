...
@interface OMOMyBookingList ()<UIScrollViewDelegate>
...
@implementation OMOMyBookingList
...
- (void)viewDidLoad {
    ...
    [self.bar addTitltLabelWithText:@"我的预约" Font:navTitleFont];
...
- (void)setUpChildViewController{
    ...
    OMORemoteBookingListVC *onlineVC = [[OMORemoteBookingListVC alloc]init];
    [self addChildViewController:onlineVC];
    
    OMOStoreBookingListVC *offlineVC = [[OMOStoreBookingListVC alloc]init];
    [self addChildViewController:offlineVC];
}
#pragma mark ---------- 顶部滑动titleview --------
- (void)omo_initHeadTitleView{
    
    UIView *titlesView = [[UIView alloc]initWithFrame:CGRectMake(lwb_margin * 2, IphoneY + lwb_margin, SCREENW - lwb_margin * 4, 44)];
    titlesView.layer.cornerRadius = lwb_smallMargin;
    titlesView.backgroundColor = WHITECOLORA(1);
    self.titlesView = titlesView;
    
    NSArray *titles = @[@"远程预约",@"门店预约"];
...
- (void)setScrollView{
    ...
    //     给本页面加载一个滚动视图
    UIScrollView *scroll = [[UIScrollView alloc]initWithFrame:CGRectMake(0, self.titlesView.lwb_bottom + 2, SCREENW, SCREENH - self.titlesView.lwb_bottom - 2)];
...
- (void)addChildVcView{
    ...
    if(index == 0){
        ...
        OMORemoteBookingListVC *firstVC = (OMORemoteBookingListVC *)self.childViewControllers[index];
        ...
        OMOStoreBookingListVC *childVC = (OMOStoreBookingListVC *)self.childViewControllers[index];
